# -*- coding: utf-8 -*-
"""===============================================================================================
The Python code of VDSR to realize SR. The idea is cited from "Kim J ,  Lee J K ,  Lee K M . 
Accurate Image Super-Resolution Using Very Deep Convolutional Networks[J]. IEEE Conference on 
...
class VDSR(BaseModel):
    ...
    def __init__(self,config,sess,save_path):
        ...
        else:self.save_path = save_path#'./TRAINED_MODEL/VDSRx{}/'.format(scale)
        
    def vdsr(self,x,scope = 'VDSR',depth = 15,reuse = None):
        with tf.variable_scope(scope,reuse = reuse):
            with slim.arg_scope([slim.conv2d],activation_fn = super().lrelu):
                ...
                y = slim.conv2d(y,1,3,stride = 1,padding = 'SAME',scope = 'conv{}'.format(depth))
                return y+x
    ...
    def train(self,
        ...
        output = self.vdsr(x_train_ph)
        test_output = self.vdsr(x_test_ph,reuse = True)
    ...
    def test(self,image = None,image_path = None):
        ...
        output = self.vdsr(input_ph,reuse = True)
        saver = tf.train.Saver()
        self.sess.run(tf.global_variables_initializer())
    ...
    def SpCodeVDSRSR(self,image_path = None,downscale_fn = None):
        ...
        output = self.vdsr(input_ph,reuse = tf.AUTO_REUSE)
        saver = tf.train.Saver()
        self.sess.run(tf.global_variables_initializer())
        saver.restore(self.sess, self.save_path+'model.ckpt')
        t = time()
